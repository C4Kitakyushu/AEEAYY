...
module.exports = {
  ...
  name: 'poli',  // Command name
  description: 'Generates an image based on the prompt provided.',  // Description 
  usage: '<prompt>',  // Usage instructions
  ...
  async execute(senderId, args, pageAccessToken) {
    ...
    const apiUrl = `https://appjonellccapis.zapto.org/api/imgen?prompt=${encodeURIComponent(prompt)}`;  // API endpoint with the prompt
    ...
    try {
      ...
      await sendMessage(senderId, {
        attachment: {
          type: 'image',
          payload: {
            url: response.data.url  // URL of the generated image
          }
        }
      }, pageAccessToken);
