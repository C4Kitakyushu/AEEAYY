...
const handleChatResponse = async (senderId, input, pageAccessToken) => {
  const systemRole = 'you are Tr1pZzey AI. an AI assistant.';
  const prompt = `${systemRole}\n${input}`;
  const apiUrl = `https://api.joshweb.click/gpt4?prompt=${encodeURIComponent(prompt)}&uid=${senderId}`;

  try {
    const { data } = await axios.get(apiUrl);
    const responseText = data.gpt4 || '☹️ No response from the API.';
